...
- 👮 **安全合规与恶意阻断**：
  - **P2P/BT 滥用检测**：实时自动诊断用户访问网站，检测 Tracker/Torrent 等敏感词，对版权滥用行为进行醒目的红色告警。
  - **系统级防火墙联动**：与系统防火墙（**UFW / Firewalld / IPTables**）深度打通，支持在 Web 端一键将违规 IP 拉入黑名单（支持 1天、1周、1月及永久封禁配置）。
  - **一键断流踢下线**：可智能定位并秒级强制切断指定在线 IP 的所有活跃连接，强行踢下线并阻断通讯端口。
  - **黑名单同步与自动清理**：黑名单持久化存入 MySQL，面板启动时自动恢复防火墙规则，并定时自动清理到期的 IP 封禁。
- 🚀 **高度自动化运维**：
